	...
	public void run() {
		try {
	        ...
	        for(int i=0;i<numberOfDrinks;i++) {
				...
				if(!Constants.findQ){Constants.waitingTime.tick(this.ID);}
				if (i==0) {
					if(!Constants.findQ){Constants.responseTime.tick(this.ID);}
					Constants.turnaroundTime.tick(this.ID);
				}

				drinksOrder[i].waitForOrder();

				if(i==0 && !Constants.findQ){Constants.responseTime.tock(this.ID);}

				System.out.println("Drinking patron " + drinksOrder[i].toString());
				sleep(drinksOrder[i].getImbibingTime()); //drinking drink = "IO"
			}

			Constants.turnaroundTime.tock(this.ID);
			if(!Constants.findQ){Constants.throughput.tock(this.ID);}
