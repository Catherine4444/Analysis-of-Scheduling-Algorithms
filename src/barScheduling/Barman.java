...
public class Barman extends Thread {
	...
	public void run() {
		...
		try {
			...
			if (!Constants.findQ){
				Constants.throughput.recordStart(); 
				Constants.cpuUtilization.recordStart(); // record start for calculating value per interval 
			}
			...
			if ((schedAlg==0)||(schedAlg==1)) { //FCFS and non-preemptive SJF
				while(true) {
					...
					int patronID = currentOrder.getOrdererID();

					Constants.waitingTime.tock(patronID);
					Constants.cpuUtilization.tick(patronID);

					sleep(currentOrder.getExecutionTime()); //processing order (="CPU burst")

					Constants.cpuUtilization.tock(patronID);
				...
				while(true) {
					...
					if(burst<=q) { //within the quantum
						...
						if(!Constants.findQ){Constants.cpuUtilization.tock(patronID);}
						...
						if(!Constants.findQ){
							Constants.cpuUtilization.tock(patronID);
							Constants.waitingTime.tick(patronID);
						}
			...
			if(!Constants.findQ){
				Constants.cpuUtilization.recordEnd();
				Constants.throughput.recordEnd();
			}
